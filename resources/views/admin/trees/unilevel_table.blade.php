...
@section('content')
    <div class="page-wrapper">
        <div class="content">
            ...
            @php
                $totalMembers = collect($data['levelCounts'] ?? [])
                    ->sum('total_members');

                $totalActiveMembers = collect($data['levelCounts'] ?? [])
                    ->sum('active_members');

                $totalIncome = $totalActiveMembers * 30;
                        ...
                        <table class="table table-bordered table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                @for($i = 1; $i <= 10; $i++)
                                    ...
                                    @php
                                        $members = $data['levelCounts'][$i]['total_members'] ?? 0;
                                        $activeMembers = $data['levelCounts'][$i]['active_members'] ?? 0;
                                        $income = $activeMembers * 30;
                                    @endphp
        ...
        document.getElementById('searchForm').addEventListener('submit', function (e) {
            ...
            if (userid) {
                window.location.href =
                    "{{ url('admin/unilevel-table') }}/" + userid;
            }
        });
    </script>
@endsection